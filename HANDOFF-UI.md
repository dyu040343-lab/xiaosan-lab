...
> 👉 仓库 / 服务器位置、部署流程、运维环境 → 见 [HANDOFF.md](HANDOFF.md)
...
vol_ratio     量比         ← 东财 f10（当日每分钟均量 ÷ 前 5 日每分钟均量）
chg_60d       60日涨跌幅(%) ← 东财 f24
chg_ytd       年初至今(%)   ← 东财 f25
list_date     上市日期 YYYY-MM-DD ← 东财 f26（用来剔除次新股）
turnover      换手率(%)    ← 东财官方 f8 = 成交量 ÷ 流通股本（注意：与「成交额 ÷ 流通市值」只是近似值）。
...
3b. **自选只允许两种保存方式**（用户 2026-09-16 明确否掉了"跟着链接走"，别再引入）：
...
4b. **Tab3「放量滞涨」的量价分口径不能随便改**（用户 2026-09-17 需求：「换手率高、价格还没涨起来」）：
    ```
    量价分 = 活跃分 × 蓄势分 ÷ 100            ← 乘法 = AND 语义，任一条件不满足分数立刻塌陷
    活跃分 = (P(换手率) + P(量比)) / 2          ← P() = 全池百分位 0~100
    蓄势分 = 100 − (P(|今日涨幅|) + P(|60日涨幅|)) / 2
    样本池 = 有价有流通盘 + 上市 ≥ 60 天 + 成交额 ≥ 1 亿 + 60 日跌幅 ≥ −40%
    ```
    ⚠️ **两个绝对值（|今日涨幅| / |60日涨幅|）是刻意的，不要改成带方向的涨幅**：
    - 「60 日涨幅」若带方向 → 跌得越惨分越高：沐曦股份（量比 8.36、换手 45%、60 日跌 29%）会排第一，
      但那是「放量下跌」，不是「价格没涨起来」；
    - 「今日涨幅」若带方向 → 榜首 30 会**全是当日下跌股**（实测 |今日涨幅| 中位 4.6%，铂力特 −12% 排第 3）。
      改成绝对值后 |今日涨幅| 中位 0.77%、涨跌各半 —— 这才是技术分析里的「量增价平」。
    - 「价格 ÷ 一年均价」不要用：它混淆「位置」与「路径」（腰斩后反弹 30% 的票看着仍在年均价下方）。
    🚫 也不要把榜单拆成「换手率TOP」+「价格BOTTOM」两张表 —— 那要用户自己做交集，两个 TOP30 往往交不上。
       想单看某一边，点列头排序即可（两边的列都在表里）。
    ⚠️ **主力净额是判别列，不是装饰**：放量滞涨既可能是吸筹、也可能是派发，方向看它；
       面板上「主力净流入」子榜就是给这个用的，别删。
...
.stock-panel (#stockPanel)   速查/自选 共用的「全指标表」面板（24 列）
#realtime                    KPI 4 卡 + tab 栏 + subtab + 搜索框 + 图表 + 表格
#chartNote («放量滞涨» tab)    图表下方一行口径提示（样本量 + 闸门 + 公式 + 吸筹/派发风险），其他 tab 隐藏
...
| `renderTable(stocks, field)` | 榜单表格（资金动向 9 列 / 占比 10 列 / 放量滞涨 11 列，含 ☆ 与换手率） |
| `volspanPool()` / `buildPctRank()` / `volspanRow()` / `vpCell()` / `pctSignCell()` / `VOLSPAN_*` | **放量滞涨榜**：样本池 + 量价分（口径见下面第 4b 条，**改之前务必读**） |
...
- **移动端**：断点只有 `max-width: 768px` 一个，9–24 列表格在手机上只能横向硬滚；可考虑卡片式/分组折叠。
- **24 列速查面板**：横向滚动时缺少列分组视觉提示（行情/资金/筹码三块），容易看错列。
