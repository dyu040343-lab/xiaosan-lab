...
8. **东财接口有 IP 限流**：主接口 `push2` **从这台香港服务器访问基本一直是 502**（2026-09-18 查 `radar_cron.log`：
   752 次跑完的抓取**全部**走的 `push2delay` 镜像，`via push2.` 0 次），脚本会自动降级，别把降级逻辑删了。
   实测 `push2delay` 的数据**不是明显延迟的**（收盘那次拿到的成交额高于 14:57 我直连主站拿到的值），可以放心用。
...
12. 🩸 **分页抓取的排序键必须是「与盘中变化无关」的字段**（2026-09-18 事故）：
    push2 的 `clist` 接口**实测会把 `pz` 压到 100 条/页**（传 5000 也只回 100），所以一次全市场要打 **60 页**。
    原来用 `fid=f62`（主力净额）排序 —— 那是盘中每秒都在变的字段，60 次请求之间顺序一直漂，
    分页边界上的票**被重复抓 / 被整只跳过**：实测 5917 行里 183 个代码重复、**180 只整只漏掉**
    （用户报「凯盛科技 600552 没数据」就是其中一只，东财侧它一直好着）。
    现在固定 `fid=f12`（证券代码）+ `po=0`，并在入列前**按代码去重**。
    ⚠️ 配套：**完成度校验必须用「去重后的唯一代码数」**，用原始行数会被重复行灌水 →
    把"漏了 180 只"误判成"抓全了"（这就是当时静默了两天的原因）。
    回归测试：`python3 tests/test_fetch_paging.py`（假服务端会模拟"排序漂移"这个物理现象）。
